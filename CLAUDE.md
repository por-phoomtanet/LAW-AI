...
- [x] 5.1 DB + API: เลือกโมเดล AI เองได้ (ใช้ร่วมกันทั้งแชททั่วไปและแชทกฎหมาย)
  ...
  - โมเดลผูกกับ `Conversation` แต่เปลี่ยนได้ตลอดแม้แชทไปแล้ว (ไม่ใช่แค่ตอนสร้างครั้งเดียวอีกต่อไป — ยกเลิกข้อจำกัดเดิมตามคำขอผู้ใช้ ดู FIX #18) การเปลี่ยนมีผลกับข้อความถัดไปเท่านั้น ข้อความเก่าที่ persist แล้วยังเก็บ `modelUsed` ของตัวเองไว้ตามจริงไม่เปลี่ยนย้อนหลัง
  - แชทกฎหมาย (5.5) ใช้ mechanism เดียวกันเป๊ะ เพราะยังเป็น `Conversation.modelTier` field เดิม ไม่ต้องเขียนแยก

  - [x] FIX #18: ผู้ใช้ขอให้เปลี่ยนโมเดลได้ตลอดแม้แชทไปแล้ว ไม่ใช่แค่ตอนสร้างบทสนทนาใหม่ตามที่ออกแบบไว้ตอนแรก | เพิ่ม `conversationRepository.updateModel(id, modelTier)` + `conversationService.updateModel(id, userId, modelId)` (ownership check + validate `AiModel.isActive=true` เหมือน `create()`) + `conversationController.updateModel` + route ใหม่ `PATCH /api/conversations/:id/model` (`body: t.Object({ modelId: t.String({minLength:1}) })` — บังคับต้องส่งมา ไม่ใช่ optional เหมือนตอน create เพราะเป็น action เปลี่ยนค่าตรงๆ ไม่มี "ค่าเริ่มต้น" ให้ fallback) | ฝั่ง web: `chatApi.updateModel()`, `ModelSelect` เพิ่ม prop `allowClear` (ปิดปุ่มล้างค่าตอนมีบทสนทนาอยู่แล้ว เพราะ conversation ต้องมีโมเดลผูกอยู่เสมอ ค่า null ไม่มีความหมายในเคสนี้), `ChatWindow.tsx`/`LegalChatWindow.tsx` โชว์ `ModelSelect` เสมอ (ไม่ใช่แค่ตอน `activeConversationId == null` อีกต่อไป) ผูก `value` กับ `modelTier` ของบทสนทนาที่เลือกอยู่จริง เปลี่ยนแล้วเรียก PATCH ทันที + `upsertConversation` อัปเดต state ให้ตรง
    - 🧪 test: เพิ่ม 3 เคสใน `conversations.test.ts` (owner เปลี่ยนโมเดลกลางคันได้จริง+persist ใน DB, `modelId` ปลอม → 400, คนอื่นเปลี่ยนของบทสนทนาที่ไม่ใช่ของตัวเอง → 404) → `bun test` รวม 63 pass ✅ | `bunx tsc --noEmit` (web) ผ่าน ✅
    - 📝 commit: `feat(api,web): allow changing a conversation's ai model at any time, not just at creation`
...
- [x] 5.2 Web: model picker (ใช้ component เดียวกันทั้งสองแชท)
  ...
  - `modules/chat/components/ModelSelect.tsx` (antd `Select`, ธีมมืดผ่าน `ConfigProvider`+`theme.darkAlgorithm` สโคปเฉพาะจุด — antd `Select`'s dropdown render ผ่าน portal ต่อท้าย `document.body`, `style`/`styles` prop ธรรมดาไม่ cascade ไปถึง ต้องใช้ `ConfigProvider` ครอบถึงจะย้อมทั้ง trigger และ popup panel ให้เข้ากันจริง) — เดิมโชว์เฉพาะตอน `activeConversationId == null` เท่านั้น ปัจจุบันโชว์เสมอและเปลี่ยนได้ตลอดแม้แชทไปแล้ว (ดู FIX #18)
  - `ChatWindow.tsx`: เพิ่ม `selectedModelId` state ใช้ก่อนสร้างบทสนทนา, หลังมีบทสนทนาแล้ว `ModelSelect`'s `value` ผูกกับ `modelTier` จริงของบทสนทนาที่เลือกอยู่แทน, reset `selectedModelId` กลับเป็น `null` ทุกครั้งที่กด "+ บทสนทนาใหม่"
...
- [x] 5.4 API: retrieval service
  - `services/rag/retrievalService.ts`: รับคำถามผู้ใช้ → full-text query บน `Passage.searchVector` join `Document`/`DocumentVersion` (filter `isLatest=true` เท่านั้น) → top-K (K=10) เรียงตาม `ts_rank`
  ...
  - [x] FIX #15: `plainto_tsquery` (ตามที่ระบุไว้ตอนแรก) AND ทุกคำเข้าด้วยกันโดย default — เจอจริงตอนเขียน 5.5 ว่าคำถามเต็มประโยคแบบที่ผู้ใช้จริงจะพิมพ์ เช่น "สถาบัน คืออะไร" ไม่แมตช์อะไรเลย เพราะคำท้าย ๆ ("คือ"/"อะไร") ไม่ปรากฏในตัวบทกฎหมายจริง ทำให้ AND ทั้งประโยคไม่มีทาง match แม้ "สถาบัน" คำเดียวจะ match เพียบก็ตาม | before: `plainto_tsquery('simple', query)` → after: แยกคำเองด้วย regex, กรอง operator character ที่ทำให้ `to_tsquery` parse error ออก, แล้ว join ด้วย ` | ` (OR) ผ่าน `to_tsquery('simple', orQuery)` แทน — `ts_rank` ให้น้ำหนัก row ที่แมตช์หลายคำสูงกว่าอยู่แล้วโดยธรรมชาติ ไม่ต้องกังวลเรื่อง noise
    - 🧪 test: เพิ่มเคส "สถาบัน คืออะไร" ใน `legalChatCompletion.test.ts` → เจอ context จริงหลัง fix (ก่อนแก้เจอ "ไม่พบข้อมูล" ทั้งที่ข้อมูลมีจริง) ✅
    - 📝 commit: `fix(api): use OR (not AND) full-text query for natural-language questions`
...
- [x] 5.5 API: `legalChatCompletionService.ts` (ไฟล์ใหม่ ไม่แก้ `chatCompletionService.ts` เดิมของ Phase 3) + citation validation
  - ก่อนเรียก OpenRouter: retrieve top-K passages จากคำถามล่าสุดเสมอ (แชทกฎหมายบังคับค้นทุกครั้ง ต่างจากแชททั่วไปที่ไม่มี retrieval เลย) → ประกอบ context + คำถามเป็นข้อความ **เดียวท้ายสุด** หลังประวัติสนทนา (ไม่ใช่ก่อนตามที่ร่างไว้ตอนแรก — แก้ให้ตรงกับ § AI/RAG Architecture ข้อ 5 จริงๆ: "system prompt คงที่เป็น prefix, ส่วนที่เปลี่ยนทุก query วางท้ายสุด" ถ้าใส่ context ก่อนประวัติ ประวัติสนทนาที่ควรเป็น stable prefix ข้ามรอบจะกลายเป็นส่วนที่ไม่ cache-friendly แทน) → ถ้าค้นไม่เจอเลย ใส่ note `(ไม่พบข้อมูลที่เกี่ยวข้องในคลังกฎหมายสำหรับคำถามนี้)` แทน context ให้โมเดลเห็นชัดว่าไม่มีอะไรให้อ้างอิง
  - system prompt คนละตัวกับแชททั่วไป — ใส่ **persona นักกฎหมายมืออาชีพ** ควบคู่กับบังคับใส่เลข `[n]` กำกับทุกข้อความที่อ้างจาก context ตาม § AI/RAG Architecture ข้อ 3 พร้อม disclaimer ตามเงื่อนไข dataset ต้นทาง (persona ไม่ใช่ตัวลบล้าง disclaimer — ยังต้องบอกชัดว่าเป็นข้อมูลอ้างอิงเบื้องต้น ไม่ใช่คำแนะนำทางกฎหมายที่มีผลผูกพัน)
  - Backend parse `[n]` จาก response ที่ stream มา (`extractValidatedCitations`) validate กับ passage ที่ retrieve จริงในรอบนั้น (Dev Standard #2) — เลขที่ไม่ตรง/เกินขอบเขตตัดทิ้งก่อนบันทึก, เก็บลง `Message.citations` เป็น `[{index, passageId, documentId, citationLabel}]` (อัปเดต comment ใน schema.prisma ให้ตรงชนิดข้อมูลจริง — comment เดิมอ้างถึง `chunkId` จาก design เก่าก่อน pivot มา Phase 4's `Passage`)
  - ยังใช้ `conversation.modelTier` เดิมเรียก OpenRouter (ผูกกับ 5.1) — ผู้ใช้เลือกโมเดลตอนสร้างแชทกฎหมายใหม่ได้เหมือนแชททั่วไป
  - **ตัดสินใจแล้ว**: reuse `/api/conversations/:id/messages` เดิม branch ตาม `conversation.mode` ข้างใน (diff เล็กกว่าจริง) — เพิ่ม `conversationRepository.findModeForUser()` (lookup เบาๆ แค่ mode ไม่ fetch messages ทั้งก้อน) ให้ route handler ใช้ตัดสินใจ dispatch ไป `chatCompletionService` หรือ `legalChatCompletionService`, เพิ่ม `mode` param ให้ `conversationService.create`/`POST /api/conversations` (จุดที่ 2 ที่แตะไฟล์ Phase 3 นอกจาก `modelId` — ทั้งคู่เป็น optional param ไม่กระทบ caller เดิม), เพิ่ม `citations?: object` ให้ `conversationRepository.appendMessage` (optional เช่นกัน)

  - [x] FIX #16: เทส `legalChatCompletion.test.ts` 4 จาก 5 เคส ไม่ได้ `await` อ่าน response body (`readSseEvents`) ก่อน assert — พังเฉพาะตอนรันรวมกับไฟล์เทสอื่นทั้ง suite (ไม่พังตอนรันไฟล์เดี่ยว) เพราะ `ReadableStream`'s `start()` ที่ persist ลง DB ทำงานแบบ async แยกจาก `app.handle()` promise ที่ resolve ทันทีที่สร้าง `Response` เสร็จ ไม่ได้รอ stream จบจริง — assertion เลยไป race กับงาน persist ที่ยังทำไม่เสร็จ ทำให้ query DB ไม่เจอ message ที่เพิ่ง "ควร" persist | before: `await app.handle(...)` แล้ว assert ทันที → after: เพิ่ม `await readSseEvents(response)` ก่อน assert ทุกเคส (ตาม pattern ที่ `chatCompletion.test.ts` เดิมทำถูกอยู่แล้ว)
    - 🧪 test: รัน `bun test` ซ้ำ 3 รอบติดกัน → 54 pass 0 fail คงที่ทุกรอบ (ก่อนแก้ fail แบบ intermittent เฉพาะตอนรันรวม) ✅
    - 📝 commit: `fix(api): await sse consumption before asserting in legal chat tests`
